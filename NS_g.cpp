...
NS::NS():FDQGMRES(10,I*J,1000)	//FDQGMRES parameterized constructor
{
	us=new double*[J+2];
	vs=new double*[J+2];
	D_x=new double*[J+1];
...
void NS::Press()
{
	...
	//FDQGMRES::solve(C,R,A,X,b);	//SSOR preconditioned iterations
	for(int i=0;i<R[I*J];i++) B[i]=A[i];	//initialization of the preconditioner for ILU(0)
	FDQGMRES::solve(C,R,A,B,X,b);	//ILU(0) preconditioned iterations
	//GMRES::solve(C,R,A,X,b);
